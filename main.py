...
from src.standardizer import map_column_names, normalize_categorical_values, filter_most_recent
...
def main():
    ...
    df_src1_raw = pl.DataFrame({
        ...
        "hosp_code": np.random.choice([f"HOSP_{i:02d}" for i in range(1, 11)], size=n),
        "surg_dt": pd.date_range("2025-01-01", periods=n, freq="h").strftime("%Y-%m-%d"),
    ...
    df_src2_raw = pl.DataFrame({
        ...
        "re_dt": pd.date_range("2025-01-15", periods=int(n * 0.90), freq="h").strftime("%Y-%m-%d"),
    ...
    print(df_features.filter(pl.col("surgical_duration_min_outlier_iqr")).select(["surgical_duration_min_outlier_iqr", "surgical_duration_min"]))
    ...
    for entry in ledger.merge_history:
        print(f"Step: {entry['step_name']} | Left Input Shape: {entry['left_input_shape']} | Output Shape: {entry['output_shape']} | Unmatched Pct: {entry['unmatched_pct']:.2f}%")
